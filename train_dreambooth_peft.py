...
# this code is adapted from https://github.com/huggingface/diffusers/blob/main/examples/advanced_diffusion_training/train_dreambooth_lora_sdxl_advanced.py
...
def add_peft_adapters_to_unet(args, unet):
...
def add_peft_adapters_to_text_encoder(args, text_encoder):
    text_encoder_target_modules = []
    if args.train_text_encoder:
        text_encoder_target_modules.extend(
            get_target_modules_list(text_encoder, TEXT_ENCODER_TARGET_REPLACE_MODULE)
        )
...
def load_combine_checkpoint(input_dir, pipe, adapter_name="default"):
    input_filepath = os.path.join(input_dir, PEFT_COMBINED_CKPT)
    with safetensors.safe_open(input_filepath, framework="pt", device="cpu") as f:
        metadata = f.metadata()
    state_dict = load_file(input_filepath)
    if metadata.get("library", None) is None or not metadata.get("has_config", False):
        raise ValueError(
            "This isn't a PEFT ckeckpoint or is missing the config required to create and load the adapters."
        )
    for model_name in MODEL_NAMES:
        if not hasattr(pipe, model_name):
            continue
        ...
        model_state_dict = {
            k.replace(f"{model_name}.", ""): v
            for k, v in state_dict.items()
            if model_name in k
        }
        set_peft_model_state_dict(model, model_state_dict, adapter_name=adapter_name)
        setattr(pipe, model_name, model)
    return pipe
...
def get_param_groups(
    ...
    unet_param_group_wit_lr = {
        "params": [param for param in unet.parameters() if param.requires_grad],
        "lr": args.learning_rate,
    }
    if not freeze_text_encoder:
        # different learning rate for text encoder and unet
        text_encoder_one_param_group_with_lr = {
            "params": [
                param for param in text_encoder_one.parameters() if param.requires_grad
            ],
            "weight_decay": args.adam_weight_decay_text_encoder,
            "lr": args.text_encoder_lr if args.text_encoder_lr else args.learning_rate,
        }
        text_encoder_two_param_group_with_lr = {
            "params": [
                param for param in text_encoder_two.parameters() if param.requires_grad
            ],
            "weight_decay": args.adam_weight_decay_text_encoder,
            "lr": args.text_encoder_lr if args.text_encoder_lr else args.learning_rate,
        }
        ...
        params_to_optimize = [unet_param_group_wit_lr]
...
def save_model_card(
    ...
    img_table_md = (
        f"""Below are the sample images generated by the PEFT Adapter for prompt: `{validation_prompt}`
            {' '.join(['|'].repeat(len(images)))}
            {'---'.join(['|'].repeat(len(images)))}
            |
        """
        if images
    ...
    for i, image in enumerate(images):
        ...
        img_table_md += f"![{validation_prompt}](image_{i}.png) |"
    yaml = f"""
---
tags:
- stable-diffusion-xl
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        "--add_new_tokens = True. By default, each --token_abstraction (e.g. TOK) is mapped to 2 new "
        "tokens - <si><si+1> ",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Whether to use Pivotal Tuning Inversion with PEFT targetting embedding layers"
        ),
    )

    parser.add_argument(
        "--training_new_tokens_frac",
        type=float,
        default=0.5,
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help=("The dimension of the PEFT adapter."),
    )
    parser.add_argument(
        "--peft_alpha",
        type=int,
        default=4,
        help=("The alpha of the PEFT adapter. Decides the scaling = alpha/rank."),
    )
    parser.add_argument(
        "--peft_dropout",
        type=float,
...
def main(args):
    ...
    # get PEFT models
    unet = add_peft_adapters_to_unet(args, unet)
    if args.train_text_encoder or args.add_new_tokens:
        text_encoder_one = add_peft_adapters_to_text_encoder(args, text_encoder_one)
        text_encoder_two = add_peft_adapters_to_text_encoder(args, text_encoder_two)
    ...
    if args.optimizer.lower() == "prodigy":
        ...
        if args.train_text_encoder and args.text_encoder_lr:
            ...
            # --learning_rate
            param_groups_to_optimize[1]["lr"] = args.learning_rate
            param_groups_to_optimize[2]["lr"] = args.learning_rate
    ...
    train_dataset = DreamBoothDataset(
        ...
        token_abstraction_dict=token_abstraction_dict if args.add_new_tokens else None,
    ...
    # if --add_new_tokens we need add_special_tokens to be True
    add_special_tokens = True if args.add_new_tokens else False
    ...
    elif args.add_new_tokens:  # args.add_new_tokens
        num_train_epochs_text_encoder = int(
            args.training_new_tokens_frac * args.num_train_epochs
        )
